...
const viewContent = {
  ...
  News: '<div class="view-header"><div><p class="eyebrow">IDIOTCORD updates</p><h2>News</h2><p>Launcher updates and Minecraft community notes.</p></div></div><div class="news-list"><article><span>0.1.0</span><div><strong>Authentication and updater fixes</strong><p>Microsoft login configuration now uses the current Entra client, and local update metadata is checked when no remote feed is configured.</p></div></article><article><span>0.1.0</span><div><strong>Launcher polish</strong><p>Fixed the full Savour logo display, refreshed release notes, and kept Debug, Release, and published builds aligned.</p></div></article><article><span>Next</span><div><strong>Instance services</strong><p>Native instance resolution, mod catalog integration, and richer release delivery are still being prepared.</p></div></article></div>',
